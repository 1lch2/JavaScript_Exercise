...
类似的，`Object.__proto__` 也是指向的 `Function.prototype`，所以 `Object.__proto__` 不是null，而 `Object.prototype.__proto__` 才是 null。

`Function.prototype` 自身是一个对象，它的 `__proto__` 又指向 Object.prototype;
...
`Object.prototype` 是所有普通对象的终点，相当于“原型链的最顶端”，为了让链条终止，它的 `__proto__` 被设计为 null。
...
由于JS的属性是沿着原型链向上查找，因此修改构造函数的原型上的属性，会影响所有由该构造函数构造出来的对象。
