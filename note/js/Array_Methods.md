...
splice 方法从索引 start 开始修改 array：删除 deleteCount 个元素并在当前位置插入 elem1, ..., elemN。最后返回已被删除元素的数组。
...
当 deleteCount 设为 0 时，splice 可以添加元素.
...
JavaScript**不允许**使用负数索引从后往前索引，但在某些数组方法中可以使用负数索引，例如上文的 splice。
...
// 或者
let arr = new Array(3).fill(0).map((val) => {
  return {};
});
...
Array.from() 是一个静态方法，从一个类似数组或可迭代对象创建一个新的，浅拷贝的数组实例。常用于将任何有长度属性或可迭代的数据结构转换为真正的数组。
