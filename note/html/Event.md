...
这里的顺序和事件冒泡相反。如上例中的 HTML，点击`<div>`后，事件发生顺序如下：
...
2. （DOM0 方式）每个元素(包括 window 和 document)都有通常小写的事件处理程序属性，比如 onclick。只要把这个属性赋值为一个函数即可:
   ...
   ```
3. （DOM2 方式）`addEventListener()` 和 `removeEventListener()`。
...
> blur 和 focus 事件不会冒泡，为了实现事件委托机制，有两种办法：
>
> 1. 通过在支持的浏览器上使用 focusin 事件
> 2. 通过设置 addEventListener() 的参数 useCapture 值为 true。示例如下：
>
> ```html
> <form id="form">
