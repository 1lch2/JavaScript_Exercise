...
/**
 * 
 * @copyright https://leetcode-cn.com/problems/copy-list-with-random-pointer/solution/
 * fu-zhi-dai-sui-ji-zhi-zhen-de-lian-biao-rblsf/1040258
 * @param {Node} head
 * @return {Node}
 */
var copyRandomList = function(head) {
  ...
  // map 存储每个原节点到复制后节点的映射
  let nodeMap = new Map();
  let temp = head;

  // 第一轮遍历存储所有原节点和新节点的映射
  while(temp != null) {
    nodeMap.set(temp, new Node(temp.val));
    temp = temp.next;
  }

  temp = head;
  // 第二轮遍历把原节点的属性依次赋值给新节点
  while(temp != null) {
    // 注意 get 结果不存在时需要赋值 null，否则会出现 undefined
