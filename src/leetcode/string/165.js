...
var compareVersion = function(version1, version2) {
  ...
  let lengthDiff = v1.length - v2.length;

  if(lengthDiff < 0) {
    v1.push(...Array.from(new Array(-lengthDiff)).map(() => "0"));
  } else if(lengthDiff > 0) {
    v2.push(...Array.from(new Array(lengthDiff)).map(() => "0"));
  }

  const compare = (v1s, v2s) => {
    if((+v1s) > (+v2s)) {
      return 1;
    } else if((+v1s) < (+v2s)) {
      return -1;
