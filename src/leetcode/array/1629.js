...
var slowestKey = function(releaseTimes, keysPressed) {
  let pressDuration = [];
  pressDuration[0] = releaseTimes[0];

  let maxIndex = 0;
  for(let i = 1; i < releaseTimes.length; i++) {
    pressDuration[i] = releaseTimes[i] - releaseTimes[i - 1];

    if(pressDuration[i] > pressDuration[maxIndex]) {
      maxIndex = i;
    }

    if(pressDuration[i] === pressDuration[maxIndex] && keysPressed[i] > keysPressed[maxIndex]) {
      maxIndex = i;
    }
  }
